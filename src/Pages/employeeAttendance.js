...
function Eattdetails() {
  ...
  const numberOfDays =
    currMonth === 1
      ? currentYear % 4 === 0
        ? 29
        : 28
      : sajn.includes(currentMonth)
      ? 30
      : 31;

  const calendarData = new Array(numberOfDays).fill().map((i, n) => n + 1);
  ...
  const firstDayOfMonth = () => {
    const a = new Date(`${monthList[currentMonth]} 1, ${currentYear} 01:15:00 `);
    const d = a.getDay();
    return d;
  };
  const list1 = new Array(firstDayOfMonth()).fill().map((i, n) => "");
  const list2 = [...list1, ...calendarData];
      ...
      <div style={{ display: "flex", flexDirection: "row" }}>
        ...
        <Header filterFn={filterFn}/>
          <div className="fltrdiv" style={{ width: "95%", marginLeft: "2.5%" }}>
            ...
            <select className="selemp1" value={currentMonth} onChange={(e)=>setCurrMonth(e.target.value)}>
              <option selected disabled>
                Month
              </option>
